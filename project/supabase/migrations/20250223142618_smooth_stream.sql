/*
  # Field Staff Table Updates
  
  1. Changes
    - Safely adds missing columns if they don't exist
    - Adds constraints and indexes if they don't exist
    - Updates RLS policies
  
  2. Security
    - Enables RLS
    - Adds policies for companies, field staff, and admins
*/
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'field_staff_company_id_staff_code_key'
  ) THEN
    ALTER TABLE field_staff ADD CONSTRAINT field_staff_company_id_staff_code_key UNIQUE (company_id, staff_code);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'field_staff_company_id_email_key'
  ) THEN
    ALTER TABLE field_staff ADD CONSTRAINT field_staff_company_id_email_key UNIQUE (company_id, email);
  END IF;
END $$;
